...
export default {
    ...
    this.storage = new StorageInterface(function(res) {
      ...
      if(atom.config.get('snippet-injector.database.enable')) {
        _this.db = new CouchDB({
          ...
          username: atom.config.get('snippet-injector.database.username'),
          password: atom.config.get('snippet-injector.database.password')
        });
      }
  ...
  insert() {
    ...
    if(snippets.length > 0) {
      ...
      atom.notifications.addWarning('No snippets in storage. Aborting.', null);
    }
  },
      ...
      const multiSearchPrompt = Util.promptMultiSearch({
        ...
        if(Util.isset(elements,'object') && elements.length) {
          ...
          if(IMEX.export(snippets)) {
            atom.notifications.addSuccess('Snippet(s) has successfully been exported.', null);
          } else {
            atom.notifications.addWarning('Something went wrong while exporting the snippet(s).', null);
          }
        }
      });
    } else {
  ...
  expall() {
    if(IMEX.export(this.storage.data())) {
      atom.notifications.addSuccess('Storage has successfully been exported.', null);
    } else {
      atom.notifications.addWarning('Something went wrong while exporting the storage.', null);
    }
  },
  ...
  imp() {
    ...
    if(Util.isset(res,'object')) {
      ...
      if(count > 0) {
        ...
        atom.notifications.addInfo('No snippets have been imported.', null);
      }
    } else {
