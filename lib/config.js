...
export default class Config {
  ...
  static setDefaults() {
    ...
    for(var key in data) {
      if(data.hasOwnProperty(key) && atom.config.get('snippet-injector.'+key) === undefined) {
        atom.config.set('snippet-injector.'+key,data[key].default);
      }
    }
  }

  static addListener(key,listener) {
    if(Util.isset(key,'string') && Util.isset(listener,'function')) {
      return atom.config.onDidChange('snippet-injector.'+key, {}, listener);
    } else {
      return false;
    }
  }
