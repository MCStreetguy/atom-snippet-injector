    ...
    if(Util.isset(state,'object')) {
      if(Util.isset(state.directory,'string')) {
        ...
        if(Util.isset(state.onAfterInit,'function')) {
          this._reload(state.onAfterInit);
        } else {
      ...
      if(Util.isset(state.autosave,'number') && state.autosave > 0) {
        this._autosave_pid = setInterval(this._save, state.autosave*1000, true);
        console.log(this._autosave_pid);
      }
    }
  }
