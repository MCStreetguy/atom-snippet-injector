...
export default class StorageInterface {
    ...
    if(Util.isset(state,'object')) {
      ...
      if(state.db.enable && Util.isset(state.db.host,'string') && Util.isset(state.db.port,'number') && Util.isset(state.db.user,'string') && Util.isset(state.db.pass,'string')) {
        this.db = new CouchDB({
          host: state.db.host,
          port: state.db.port,
          username: state.db.user,
          password: state.db.pass
        });

        try {
          this.initDatabase();
        } catch(e) {
          atom.notifications.addWarning('An Error occured while initializing database connection!', {
            detail: 'CouchDB module threw: '+e.message
          });
          this.db = undefined;
        }
      }
  ...
  _db_error(msg) {
    ...
    var not = atom.notifications.addError('An Error occured while initializing database connection!', {
      detail: 'CouchDB module threw: '+msg,
      dismissable: true,
      icon: 'database',
      buttons: [
        {
  ...
  initDatabase() {
    ...
    if(Util.isset(_this.db,'object')) {
      cdb.allDbs(function(data) {
        if(data.status === 'done') {
          if(data.response.indexOf('snippet-injector-data') < 0) {
            var res = cdb.createDb('snippet-injector-data');
            if(!res.ok) {
              _this._db_error('Unable to create database snippet-injector-data!');
              _this.db = undefined;
            }
          } else {
            cdb.getDb('snippet-injector-data');
          }
        } else {
          _this._db_error('Unable to retrieve databases from the server!');
        }
      })
    }
  }
