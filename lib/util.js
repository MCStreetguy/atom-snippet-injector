...
const _append = function(elem,html) {
  elem.innerHTML = elem.innerHTML + html;
}

const _prepend = function(elem,html) {
  elem.innerHTML = html + elem.innerHTML;
}

const _hasClass = function(elem,classname) {
...
const _toggleClass = function(elem,classname) {
  if(_hasClass(elem,classname)) {
...
const _removeAllListeners = function(elem) {
...
export default class Util {
  ...
  static promptUser(options,callback) {
    if(typeof options !== 'object') {
      ...
      if(options.title !== undefined && options.title !== null) {
        _append(inputPrompt,'<h4 class="title">'+options.title+'</h4>');
      }

      var edit = atom.workspace.buildTextEditor({mini: true});
      edit.setPlaceholderText(''+options.placeholder);
      _append(inputPrompt,atom.views.getView(edit));

      editElem = document.querySelector('#input-prompt atom-text-editor');
      editElem.id = 'prompt-input';

      _append(inputPrompt,'<button class="btn btn-success disabled" id="prompt-confirm">'+options.btnconfirm+'</button><button class="btn btn-default" id="prompt-cancel">'+options.btncancel+'</button>');
      var confirmbtn = document.querySelector('#input-prompt button#prompt-confirm');
      var cancelbtn = document.querySelector('#input-prompt button#prompt-cancel');

      _append(inputPrompt,'<div class="more-options"><input type="checkbox" id="is-shared"> Synchronize with database</div>');
      if(!atom.config.get('snippet-injector.database.enable')) {
        var moreoptions = document.querySelector('#input-prompt .more-options');
        _addClass(moreoptions,'hidden');
        inputPrompt.style.height = '30px';
      }

      edit.onDidChange(function() {
        if((edit.getText() === '' || edit.getText() === ' ' || edit.getText() === null || edit.getText() === undefined)) {
          _addClass(confirmbtn,'disabled');
        } else {
          _removeClass(confirmbtn,'disabled');
        }
      });
      var handleConfirm = function(e) {
        if(!_hasClass(confirmbtn,'disabled')) {
  ...
  static promptMultiSearch(options,callback) {
    if(typeof options !== 'object') {
      ...
      if(options.title !== undefined && options.title !== null) {
        _append(searchPrompt,'<h4 class="title">'+options.title+'</h4>')
      }
      var edit = atom.workspace.buildTextEditor({mini: true});
      edit.setPlaceholderText(''+options.placeholder);

      _addClass(searchPrompt,'select-list');
      _append(searchPrompt,atom.views.getView(edit));
      var editElem = document.querySelector('#search-multi-prompt atom-text-editor');
      editElem.id = 'prompt-search';
      ...
      _append(searchPrompt,listDOMstring);
      var promptlist = searchPrompt.querySelector('ol#prompt-list');
      var confirmbtn = searchPrompt.querySelector('button#prompt-confirm');
      var cancelbtn = searchPrompt.querySelector('button#prompt-cancel');
      IconHelper.colorize('colored-svg');

      edit.onDidChange(function() {
        var text = edit.getText();
        _removeAllListeners(promptlist);
        document.querySelectorAll('#prompt-list .list-item, #prompt-list .notice').forEach(function(e) {
          e.remove();
        });
        ...
        _append(promptlist,listDOMstring);
        IconHelper.colorize('colored-svg');
        ...
        promptlist.addEventListener('click',function(e) {
          if(_hasClass(e.target,'list-item')) {
            _toggleClass(e.target,'selected');
            var id = e.target.querySelector('.value').getAttribute('data-id');
      ...
      promptlist.addEventListener('click',function(e) {
        if(_hasClass(e.target,'list-item')) {
          ...
          _toggleClass(confirmbtn,'disabled');
