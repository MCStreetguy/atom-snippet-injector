...
export default class Util {
  ...
  static promptMultiSearch(options,callback) {
    if(typeof options !== 'object') {
      ...
      listDOMstring += '</ol><div id="secret_storage_element" class="hidden"></div><button class="btn btn-success disabled" id="prompt-confirm">'+options.btnconfirm+'</button>\
      <button class="btn btn-default" id="prompt-cancel">'+options.btncancel+'</button>';
      ...
      edit.onDidChange(function() {
        ...
        $('#prompt-list').on('click','.list-item',function(e) {
          if($('#secret_storage_element').children().length && $('#secret_storage_element').children().find(this)) {
            $('#secret_storage_element').find(this).remove();
          } else {
            $('#secret_storage_element').append(this);
          }
          $('#prompt-confirm').toggleClass('disabled',($('#secret_storage_element').children().length < 1));
        });
      });
      var handleConfirm = function(e) {
        if(!$('#prompt-confirm').hasClass('disabled')) {
          ...
          var selected = [];
          $('#secret_storage_element').children().each(function(e) {
